<?php $usersidd = $user['id'];
$vids=mysqli_query($sql, "SELECT * FROM `favorites` WHERE userid = '$usersidd' ORDER BY `id` DESC LIMIT 1");
while($vides=mysqli_fetch_array($vids)) { ?>
	...
	<b class="rch5"></b>
		...
		<div class="headerTitle">
					<div class="headerTitleRight">
						...
						(<a href="/user/<?php echo htmlspecialchars($username); ?>/favorites" class="headers">View All Favorites</a>)
				...
				<td>
					<table width="500" height="121" style="background-color: #ffffff; " cellpadding="0" cellspacing="0">
						<tr>
							...
							<?php $vids=mysqli_query($sql, "SELECT * FROM `favorites` WHERE userid = '$usersidd' ORDER BY `id` DESC LIMIT 4");
							while($vide=mysqli_fetch_array($vids)) { ?>
							<?php $vididfav = $vide['video']; ?>
							<?php $vids=mysqli_query($sql, "SELECT * FROM `video` WHERE id = '$vididfav' AND DELETED = 0 ORDER BY `id` DESC LIMIT 1");
